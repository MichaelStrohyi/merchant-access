...
class Store
{
    ...
    {
        # return false if primary logo was modified and not saved
        if  ($this->primaryLogoNotSaved()) {
            return false;
        }

        # return true if store vars were not modified
        if (!$this->isModified) {
            return true;
        }
    ...
    /**
     * Return true if primary logo is modified and not saved. Otherwise return false.
     *
     * @return boolean
     * @author Michael Strohyi
     **/
    public function primaryLogoNotSaved()
    {
        if ($this->exists() && $this->getPrimaryLogo()->getIsModified() && !$this->getPrimaryLogo()->save()) {
            return true;
        }
